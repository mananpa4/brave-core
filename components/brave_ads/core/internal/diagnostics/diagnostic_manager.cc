...
std::optional<bool> IsClientStateValid() {
  return ClientStateManager::GetInstance().IsValid();
}
...
DiagnosticManager::DiagnosticManager() {
  ...
  SetEntry(std::make_unique<OptionalBoolDiagnosticEntry>(
      DiagnosticEntryType::kClientStateValid, "Client state valid",
      base::BindRepeating(&IsClientStateValid)));
...
void DiagnosticManager::TestDiagnosticsConditionMatcher(
  ...
  ad_events_database_table.GetVirtualPrefs(
      ...
      base::BindOnce(&BuildTestDiagnosticsConditionMatcherResultCallback,
