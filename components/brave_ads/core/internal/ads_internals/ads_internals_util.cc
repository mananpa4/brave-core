...
void GetTransactionsForInternalsCallback(GetInternalsCallback callback,
  ...
  ads_internals.transactions = std::move(transactions);
  Successful(std::move(callback), std::move(ads_internals));
}

void GetConditionMatchersCallback(GetInternalsCallback callback,
                                  AdsInternalsInfo ads_internals,
                                  bool success,
  ...
  ads_internals.condition_matchers = std::move(condition_matchers);
  DiagnosticManager::GetTransactions(
      base::BindOnce(&GetTransactionsForInternalsCallback, std::move(callback),
                     std::move(ads_internals)));
}

void GetNewTabPageAdCampaignsCallback(GetInternalsCallback callback,
                                      AdsInternalsInfo ads_internals,
                                      bool success,
  ...
  ads_internals.active_new_tab_page_ad_count = active_new_tab_page_ad_count;
  ads_internals.active_new_tab_page_ad_campaigns = std::move(campaigns);
  DiagnosticManager::GetConditionMatchers(
      base::BindOnce(&GetConditionMatchersCallback, std::move(callback),
                     std::move(ads_internals)));
}

void GetNotificationAdCampaignsCallback(GetInternalsCallback callback,
                                        AdsInternalsInfo ads_internals,
                                        bool success,
  ...
  ads_internals.active_notification_ad_count = active_notification_ad_count;
  ads_internals.active_notification_ad_campaigns = std::move(campaigns);
  DiagnosticManager::GetNewTabPageAdCampaigns(
      base::BindOnce(&GetNewTabPageAdCampaignsCallback, std::move(callback),
                     std::move(ads_internals)));
}

void GetPaymentTokensForInternalsCallback(GetInternalsCallback callback,
                                          AdsInternalsInfo ads_internals,
                                          bool success,
  ...
  ads_internals.payment_tokens = std::move(payment_tokens);
...
void GetConfirmationQueueForInternalsCallback(
  ...
  ads_internals.confirmation_queue = std::move(confirmation_queue);
  DiagnosticManager::GetPaymentTokens(
      base::BindOnce(&GetPaymentTokensForInternalsCallback, std::move(callback),
                     std::move(ads_internals)));
}
...
void GetUnexpiredAdEventsCallback(GetInternalsCallback callback,
  ...
  DiagnosticManager::GetConfirmationQueue(
      base::BindOnce(&GetConfirmationQueueForInternalsCallback,
                     std::move(callback), std::move(ads_internals)));
}
