...
AdsInternalsHandler::AdsInternalsHandler(
  ...
  pref_change_registrar_.Add(
      ...
      base::BindRepeating(&AdsInternalsHandler::OnWalletConnectedPrefChanged,
                          weak_ptr_factory_.GetWeakPtr()));
...
void AdsInternalsHandler::TestDiagnosticsConditionMatcher(
  ...
  ads_service_->TestDiagnosticsConditionMatcher(
...
void AdsInternalsHandler::OnWalletConnectedPrefChanged(
    const std::string& /*path*/) {
  UpdateWalletConnected();
}

void AdsInternalsHandler::UpdateWalletConnected() {
  if (!ads_internals_page_remote_) {
    return;
  }
  ...
  ads_internals_page_remote_->UpdateWalletConnected(is_connected);
}
