...
void JsonRpcService::GetAllNetworks(GetAllNetworksCallback callback) {
  ...
  for (auto coin : GetEnabledCoins()) {
    for (const auto& chain : NetworkManager::GetAllKnownChains(coin)) {
      result->known_chain_ids.push_back(chain->chain_id);
    }
  ...
  for (const auto& entry : kAnkrBlockchains) {
    result->ankr_chain_ids.emplace_back(entry.first);
  }
